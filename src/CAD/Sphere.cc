...
Sphere::Sphere(Canvas *c, const Style *s, uint32_t latRes, uint32_t longRes,
               uint32_t radius)
    : Shape(c), style(s), latRes(latRes), longRes(longRes), radius(radius) {}
Sphere::~Sphere() {}
...
void Sphere::init() {
  ...
  //    ind.push_back(i + longRes);
  //    ind.push_back((i % longRes == longRes - 1 ? -longRes : 0) + i + 1);
  //    ind.push_back((i % longRes == longRes - 1 ? -longRes : 0) + i + longRes
  //    + 1);
  //  }
  ...
  for (int i = 1; i < longRes; i++) addTri(S, i - 1, i);
  ...
  //  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(uint32_t)*ind.size(),
