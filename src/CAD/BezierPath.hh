...
class BezierPath : public Shape2D {
 ...
 public:
  BezierPath(Vec3D p1, Vec3D p2, Vec3D p3, Vec3D p4, Canvas* c, Style* s)
      : Shape2D(c, p1.x, p1.y, s), p1(p1), p2(p2), p3(p3), p4(p4) {
    this->getPoints();
    this->drawingPoints = this->unwrap(this->points);
  }

  void getEquationX(double t, const Vec3D& p0, const Vec3D& p1, const Vec3D& p2,
                    const Vec3D& p3);
  void getEquationY(double t, const Vec3D& p0, const Vec3D& p1, const Vec3D& p2,
                    const Vec3D& p3);
  void getEquationZ(double t, const Vec3D& p0, const Vec3D& p1, const Vec3D& p2,
                    const Vec3D& p3);
  float x(double t);
  float y(double t);
  float z(double t);
