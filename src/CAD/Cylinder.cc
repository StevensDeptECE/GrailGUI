...
// Cylinder::Cylinder(uint32_t height, uint32_t r1, uint32_t r2, uint32_t res) :
// height(height), r1(r1), r2(r2), res(res){
// }
Cylinder::Cylinder(Canvas* c, const Style* s, uint32_t height, uint32_t x,
                   uint32_t y, uint32_t z, uint32_t radius, uint32_t segments)
    : Shape(c),
      style(s),
      height(height),
      radius(radius),
...
inline void Cylinder::getUnitCircleVertices() {
  // This is to generate the unit circle on the XY plane that will be used to
  // help render the cylinder
...
void Cylinder::init() {
  ...
  for (int i = 0; i < 2; ++i) {
    float h = height * (i * 2);
    // Variable k will be used to access the unit circle at the current
    // iteration.
