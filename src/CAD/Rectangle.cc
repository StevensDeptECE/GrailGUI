...
Rectangle::Rectangle(Canvas* c, float x, float y, float width, float height, const Style* s) :
    Shape2D(c, x, y, s) , x1(x) , y1(y) , x2(x + width) , y2(y) , x3(x) , y3(y + height) , x4(x + width), y4(y + height){}//, x5(x+width), y5(y){}//, x6(x), y6(y+height){} 
