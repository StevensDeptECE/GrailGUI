...
/*
  not using this yet. We can't figure out a convenient way to share the code between
  the three vectors, so it's cut and past in the load file for now
 */
  template<typename T>
  void loadCSRFile(const char filename[], uint64_t fileLen) {
    ...
    if(posix_memalign((void **)&csrVec, getpagesize(), sizeof(T)*(fileLen+1)))
...
public:
  VertexIndex_t startIndexLen; // this is actually the number of vertices (V) the actual length = +1
  ...
  Weight_t* weight;
  CSRGraph() : Graph(), startIndexLen(0), adjLen(0), weightLen(0),
               startIndex(nullptr), adjacency(nullptr), weight(nullptr) {}
	~CSRGraph(){
  ...
  CSRGraph(const char csrBeginName[], const char csrAdjName[], const char weightName[]) {
    double tm = wtime();
    FILE *file = nullptr;
    size_t ret;
	
    startIndexLen = fsize(csrBeginName) / sizeof(EdgeIndex_t) - 1; // one extra index for last one
    adjLen = fsize(csrAdjName) / sizeof(VertexIndex_t);
   
    // Read CSR starting Index
    file = fopen(csrBeginName, "rb");
    if (file == nullptr)
      throw "fopen fail csrbegin";
      //      throw Ex(__FILE__, __LINE__, "Can't open");
    startIndex = nullptr;
    
    //if(posix_memalign((void **)&startIndex, getpagesize(), sizeof(EdgeIndex_t)*(startIndexLen+1)))
    ...
    if (file == nullptr)
      throw "fopen fail adj";
      //      throw Ex(__FILE__, __LINE__, "Can't open");
    adjacency = nullptr;
    
    //if(posix_memalign((void **)&adjacency, getpagesize(), sizeof(VertexIndex_t)*(adjLen)))
  ...
  uint32_t* computeHistogram() const {
    ...
    for (VertexIndex_t v = 0; v < getV(); v++)
      hist[countAdjacencies(v)]++;
    return hist;
  }
  ...
  struct VertexCount {
    ...
    VertexCount(VertexIndex_t v, VertexIndex_t countAdj) : v(v), countAdj(countAdj) {}
  };
