...
//  static bool hasBeenInitialized;
  ...
  static void saveFonts(const uint8_t* combinedBitmap, uint32_t sizeX, uint32_t sizeY);
...
/* Build a font face by
    ...
    storing the rendered characters into a single large bitmap in row-major scan order
*/
  FontFace(FT_Library ft,
           const std::string& faceName, const std::string &facePath,
  ...
  static const Font* get(const char faceName[], uint32_t size, uint32_t boldness);
...
class Font {
 ...
 public:
  class Glyph {
   public:
    ...
    Glyph(float advance, float bearingX, float bearingY, float sizeX, float sizeY,
         float u0, float u1, float v1, float v0)
        : advance(advance),
          bearingX(bearingX),
          bearingY(bearingY),
          sizeX(sizeX),
 ...
 private:
  FontFace* parentFace;
  //TODO:uint32_t parentFace;   // offset of parent into the static fontface table for ease of serialization to disk
  uint16_t height;       // height of rectangle for each glyph of this font in pixels
  uint32_t startGlyph;   // first character to be written, 32 bits to handle any char
  //uint32_t numGlyphs;    // number of glyphs supported, some might be the same
  uint32_t textureId;
  std::vector<Glyph> glyphs;
 public:
  uint32_t maxWidth;    // biggest width of any glyph
  uint16_t spaceWidth;  // width of a space character
  Font(FontFace* Face, FT_Face ftFace, uint16_t height,
  ...
  void addGlyph(FT_Face ftFace, std::unordered_map<uint32_t, uint32_t>& glyphMap, uint8_t c, uint8_t bitmap[],
  ...
  friend std::ostream& operator<<(std::ostream& s, const Font& f) {
    ...
    //return s << "Font " << f.parentFace->faceName << " height=" << f.height << " numGlyphs=" << f.numGlyphs; // friend isn't working? what a pain
    //return s << "Font " << FontFace::faces[f.parentFace]->faceName << " height=" << f.height << " numGlyphs=" << f.numGlyphs;
  }
