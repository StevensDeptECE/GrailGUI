...
void Cursor::process_input(Inputs *in, float dt) {
  ...
  // This trans would be set with the deltas and then it just adds it onto the
  // transform glm::vec3 trans(0,0,0);
  ...
  if (GLWin::mouse_entered) {
    ...
    x = abs(v_x) > 1
            ? v_x * dt + 0.5 * a_x * dt * dt + 1.0 / 3 * j_x * dt * dt * dt
            : 0;
    y = abs(v_y) > 1
            ? v_y * dt + 0.5 * a_y * dt * dt + 1.0 / 3 * j_y * dt * dt * dt
            : 0;
...
void Cursor::render() {
  if (!GLWin::use_inputs_library || GLWin::mouse_leaved ||
      !GLWin::window_focused)
    return;
