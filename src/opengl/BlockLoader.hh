...
protected:
	/*
		TODO: Find better way than uninitialized parent?
		 ...
		 In any file format, there will be this problem. There must be a temporary
		 object structure while you read in and figure out how big the data is.
		 Then, once the data is known, you can read in a single contiguous block
		 of memory, fill it and save that out to disk for rapid loading later.
		 ...
		 just keep an extra data structure with the points. Then, when done growing
		 define a pointer to the points at the end and copy in.

		 The uninitialized protected constructor is only for use by children who
		 first figure out how big, then call init() to initialize their parent.
		 The only alternative would be to call the parent constructor passing 
		 in the information (type and version is easy, 
		 number of bytes requires knowing everything before call)
		 This would require returning a structure with those 3 items. Should probably just do that.
	*/
	BlockLoader() {}
	struct Info {
		uint64_t bytes;
