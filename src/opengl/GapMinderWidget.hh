...
class GapMinderWidget : public Widget2D {
 ...
 public:
  GapMinderWidget(StyledMultiShape2D* m, MultiText* t, float x, float y,
  ...
  GapMinderWidget(StyledMultiShape2D* m, MultiText* t, float x, float y,
                  float w, float h)
      : GapMinderWidget(m, t, x, y, w, h, std::string(""), nullptr, nullptr, 0,
                        100, 0, 100, 1.25, 10, 10, x, new LinearScale(),
                        new LinearScale()) {}

  GapMinderWidget(StyledMultiShape2D* m, MultiText* t, float x, float y,
                  float w, float h, const std::vector<float>& xLocations,
                  const std::vector<float>& yLocations)
      : GapMinderWidget(m, t, x, y, w, h, "", nullptr, nullptr, 0, 0, 0, 0,
                        1.25, 10, 10, x, new LinearScale(), new LinearScale()) {
  ...
  void chart(const std::vector<float>& yLocations,
             const std::vector<float>& xLocations,
             const std::vector<float>& sizes, int rulerIntervalX,
             int rulerIntervalY, const std::vector<glm::vec4>& c);
  void loadData(std::string sy, std::string sx, std::string ss, int startYear,
                int endYear);
