...
/*
  WebCursor represents the current location within a Shape, which  can be
  updated in order to stream visual objects into a rectangular region
 */
class WebCursor {
 private:
  Canvas* c;
 ...
 public:
  WebCursor(Canvas* c, MultiShape2D* s) : c(c), s(s) {
    ...
    std::cout << "Canvas dimensions: (" << width << ", " << height << ")\n";
  }

  WebCursor(Canvas* c, MultiText* m) : c(c), m(m) {
    x = 0.0f;
    y = 0.0f;
