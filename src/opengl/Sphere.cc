...
inline void Sphere::addRect(uint32_t i1, uint32_t i2, uint32_t i3,
                            uint32_t i4) {
  ind.push_back(i1);
  ind.push_back(i2);
  ind.push_back(i3);
...
void Sphere::init() {
  const double DPHI =
      PI<double> / latRes;  // vertically only +90 to -90 degrees
  ...
  // preallocate enough space for the central band of rectangles, each 2
  // triangles *6
  //  plus the endcaps, each latRes triangles *3 (x,y,z) * 2 (north and south
  //  poles)
  ...
  //    Next, create the indices for the triangles for each grid in the central
  //    band
  ...
  for (int i = 1; i < lonRes; i++) addTri(S, i - 1, i);
  addTri(S, lonRes - 1, 0);

  // add the indices for the north cap
  const uint32_t NORTHEDGE = (latRes - 1) * lonRes;
  for (int i = 1; i < lonRes; i++) addTri(N, NORTHEDGE + i - 1, NORTHEDGE + i);
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * vert.size(), &vert[0],
               GL_STATIC_DRAW);
