...
uint32_t StyledMultiShape25D::addSector(float x, float y, float z, float xRad, float yRad,
  ...
  for (float i = fromAngle; i <= toAngle; i += angleInc) {
    ...
    addStyledPoint(px, py, 0, c);
  }
  return (toAngle - fromAngle) / angleInc + 1;
}

StyledMultiShape25D::~StyledMultiShape25D() {}

// TODO: Maybe add a different render calls that loops through array of either
// the render of style or the super render Shape25D
void StyledMultiShape25D::render() {
...
void StyledMultiShape25D::updatePoints() {
  glBindBuffer(GL_ARRAY_BUFFER, vbo);
  glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(float), &vertices[0],
               GL_DYNAMIC_DRAW);
}

void StyledMultiShape25D::updateIndices() {
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, lbo);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLuint) * lineIndices.size(),
               &lineIndices[0], GL_DYNAMIC_DRAW);
}
...
void StyledMultiShape25D::fillRectangle(float x, float y, float z, float w, float h,
  ...
  addStyledPoint(x + w, y + h, z, c);
  addStyledPoint(x + w, y, z, c);
  sAddQuadIndices();
  points = 4;
...
void StyledMultiShape25D::fillRoundRect(float x, float y, float z, float w, float h,
  ...
  addStyledPoint(x + w / 2, y + h / 2, z, c);

  uint32_t toAdd = addSector(x + rx, y + ry, z, rx, ry, 90, 180, 1, c);
  ...
  toAdd = addSector(x + rx, y + h - ry, z, rx, ry, 180, 270, 1, c);
  sAddSectorIndices(centerIndex, toAdd);
  currentIndex += toAdd;
  points += toAdd;
  ...
  toAdd = addSector(x + w - rx, y + h - ry, z, rx, ry, 270, 360, 1, c);
  sAddSectorIndices(centerIndex, toAdd);
  currentIndex += toAdd;
  points += toAdd;
  ...
  toAdd = addSector(x + w - rx, y + ry, z, rx, ry, 0, 90, 1, c);
  sAddSectorIndices(centerIndex, toAdd);
  currentIndex += toAdd;
  points += toAdd;
...
void StyledMultiShape25D::fillEllipse(float x, float y, float z, float xRad, float yRad,
  ...
  uint32_t toAdd = addSector(x, y, z, xRad, yRad, 0, 360, angleInc, c);
  sAddSectorIndices(centerIndex, toAdd);
...
void StyledMultiShape25D::fillGrid(float x0, float y0, float z, float w, float h,
  ...
  for (uint32_t indexVert = numVert + 1; indexVert > 0; indexVert--, x += dx)
    drawLine(x, y0, z, x, y0 + h, lc);
  // draw horizontal lines
  float y = y0;
  const float dy = (h / numHoriz);
  for (uint32_t indexHoriz = numHoriz + 1; indexHoriz > 0;
       indexHoriz--, y += dy)
    drawLine(x0, y, z, x0 + w, y, lc);
}
...
void StyledMultiShape25D::fillPolygon(const float xy[], float z, uint32_t n,
  ...
  mid_x /= n;
  mid_y /= n;
  addStyledPoint(mid_x, mid_y, z, c);
  for (uint32_t i = 0, j = 0; i < n; i++, j += 2) {
...
void StyledMultiShape25D::drawPolygon(const float xy[], float z, uint32_t n,
  ...
  addStyledPoint(xy[n * 2 - 2], xy[n * 2 - 1], z, c);
  lineIndices.push_back(ind);
...
void StyledMultiShape25D::polygonPoints(float x, float y, float z, float xRad, float yRad,
  ...
  uint32_t toAdd = addSector(x, y, z, xRad, yRad, 0, 360, 360 / n, c);
  points += toAdd;
  pAddSectorIndices(centerIndex, toAdd);
...
void StyledMultiShape25D::circlePoints(float x, float y, float z, float rad,
  ...
  uint32_t toAdd = addSector(x, y, z, rad, rad, 0, 360, angleInc, c);
  points += toAdd;
  pAddSectorIndices(centerIndex, toAdd);
...
void StyledMultiShape25D::bezierSegment(const Bezier* b, float z) {
  addStyledPoint(b->getp2x(), b->getp2y(), z, b->getColor());
  addStyledPoint(b->getp3x(), b->getp3y(), z, b->getColor());
  uint32_t ind = getPointIndex();
  ...
  for (uint32_t i = 0; i < b->getn(); i++, t += b->getdt()) {
    ...
    addStyledPoint(x, y, 0, b->getColor());
    lineIndices.push_back(ind);
    lineIndices.push_back(++ind);
  }
  if (b->getEnd() == true) {
