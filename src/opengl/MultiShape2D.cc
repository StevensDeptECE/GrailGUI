...
void MultiShape2D::sAddQuadIndices() {
  ...
  solidIndices.push_back(startIndex);
  IndiceCount.push_back(6);
}

void MultiShape2D::sAddSectorIndices(uint32_t centerIndex,
                                     uint32_t indexCount) {
...
void MultiShape2D::pAddSectorIndices(uint32_t centerIndex,
                                     uint32_t indexCount) {
  uint32_t startIndex = getPointIndex() - indexCount;
  for (uint32_t curIndex = startIndex; curIndex < startIndex + indexCount - 1;
       curIndex++) {
...
void MultiShape2D::fillTriangle(float x1, float y1, float x2, float y2,
                                float x3, float y3) {
  addPoint(x1, y1);
  addPoint(x2, y2);
  addPoint(x3, y3);
...
void MultiShape2D::drawTriangle(float x1, float y1, float x2, float y2,
                                float x3, float y3) {
  addPoint(x1, y1);
  addPoint(x2, y2);
  addPoint(x3, y3);
...
void MultiShape2D::drawEllipse(float x, float y, float xRad, float yRad,
                               float angleInc) {
  uint32_t centerIndex = getPointIndex();
  addPoint(x, y);
...
void MultiShape2D::bezierSegmentByPoints(float p1x, float p1y, float p2x,
