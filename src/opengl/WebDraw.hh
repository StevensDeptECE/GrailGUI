...
/*
  WebDraw is a single class that manages OpenGL to draw many kinds of 2d shapes in different colors and fonts
  accordingly, every method has a parameter for Style from which color and font are taken
 */
...
private:
  ...
  int currentIndex;
public:
  WebDraw(uint32_t vertCount=1024,
					uint32_t solidIndCount=1024,
					uint32_t lineIndCount=1024,
					uint32_t pointIndCount=1024) :
		MultiShape2D(s,vertCount, solidIndCount, lineIndCount, pointIndCount,5), currentIndex(0) {
    startIndices.push_back(0);
  }

  uint32_t addSector(float x, float y,
										 float xRad, float yRad,
										 float fromAngle, float toAngle,
										 float angleInc, const Style * s);
  ...
  void updateColors(const uint64_t pos, const float r, const float g, const float b);
  
  //Solid Primitives
  void fillRectangle(float x, float y, float w, float h, const Style* s);
  void fillRoundRect(float x, float y, float w, float h, float rx, float ry, const Style* s);
  void fillTriangle(float x1, float y1, float x2, float y2,float x3, float y3, const Style* s);
  void fillPolygon(float x, float y, float xRad, float yRad, float n, const Style* s);
	//TODO: probably get rid of angleInc, but figure out how to control the accuracy
  void fillCircle(float x ,float y, float rad, float angleInc, const Style* s);
  void fillEllipse(float x, float y, float xRad, float yRad, float angleInc, const Style* s);

  //Line Primitives
  void drawRectangle(float x, float y, float w, float h, const Style* s);
  void drawRoundRect(float x, float y, float w, float h, float rx, float ry, const Style* s);
  void drawTriangle(float x1,float y1, float x2,float y2,
										float x3, float y3, const Style* s);
  void drawPolygon(float x, float y, float xRad, float yRad, float n, const Style* s);
  void drawCompletePolygon(float x, float y, float xRad, float yRad, float n, const Style* s);
  void drawCircle(float x ,float y, float rad, float angleInc, const Style* s);
  void drawEllipse(float x, float y, float xRad, float yRad, float angleInc, const Style* s);
  ...
  /*
    ...
    e = xy[i+4], f = xu[i+5], g = xy[i+6], h = xy[i+7]
		...
		this can be done directly from each 4 control points. For Splines,
		this requires solving a system of linear equations
		see http://mathworld.wolfram.com/Spline.html

		numSegments is the number of lines to draw per cubic piece
	*/
	void drawCubic(float xy[], uint32_t length, uint32_t numSegments);

  //TODO: How do we take Bezier and Spline objects and generate the underlying xy[] coefficients to drive the above?
  
  //Point Primitives
  void rectanglePoints(float x, float y, float w, float h, const Style* s);
  void roundRectPoints(float x, float y, float w, float h, float rx, float ry, const Style* s);
  void trianglePoints(float x1, float y1, float x2, float y2,float x3, float y3, const Style* s);
  void polygonPoints(float x, float y, float xRad, float yRad, float n, const Style* s);
  void circlePoints(float x ,float y, float rad, float angleInc, const Style* s);
  void ellipsePoints(float x, float y, float xRad, float yRad, float angleInc, const Style* s);
  ...
  // draw an entire rectangular text to a rectangle
	...
	void drawText(float x, float y, Style* s, const uint16_t text[], uint32_t len);
  ...
  // draw text at the current insert point, if it does not fit move to next line, but
  // if it doesn't
  // TODO: we need font metrics!!
    ...
    //call moveRight to update cursor position based on font size and lenght of array
    cout << "Adding text..\n";
