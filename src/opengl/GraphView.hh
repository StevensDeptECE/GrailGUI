...
class GraphView : public Shape {
 ...
 public:
  ...
  void setVertexRadius(float vertR) { this->vertR = vertR; }
  void addColor(float r, float g, float b) {
    colorMap.push_back(glm::vec4(r, g, b, 0));
  }
  void addVert(uint64_t v, float x, float y, uint8_t initialVertColor,
               Point points[]);  // each vertex starts with color value 0
  void addEdge(uint64_t i, float x1, float y1, float x2, float y2,
               uint8_t initialEdgeColor);
  void updateVert(uint64_t v,
                  uint8_t colorVal);  // change the color of each point of vert
                                      // with colorMap[colorVal]
  void updateEdge(
      uint64_t i,
      uint8_t colorVal);  // change the color of the line colorMap[colorVal]
  GraphView(CSRGraph<uint64_t, uint64_t, float>& g, Canvas* c, Style* style,
            glm::vec4 s)
      : mVertices(style), mEdges(style), g(&g), defaultStyle(s), c(c) {
    c->addLayer(&mEdges);
