...
std::vector<std::unique_ptr<ESRIShape>> ESRIShape::convertSHPObjects(const std::vector<SHPObject*>& shapeObjects) {
  ...
  for (int i = 0; i < shapeObjects.size(); i++) {
    switch (shapeObjects[i]->nSHPType) {
      case SHPT_POLYGON:
        esriShapes[i] = std::make_unique<ESRIPolygon>(ESRIPolygon(shapeObjects[i]));
        break;
      default:
