...
void StyledMultiShape2D::fillRectangle(float x, float y, float w, float h, const glm::vec4& c){
  uint32_t points = 0;
  addStyledPoint(x,y,c);  	  addStyledPoint(x,y+h,c); // goes in counter-clockwise order
  addStyledPoint(x+w, y+h,c); addStyledPoint(x+w, y,c);
  sAddQuadIndices();
  points = 4;
...
void StyledMultiShape2D::fillTriangle(float x1, float y1, float x2, float y2,float x3, float y3, const glm::vec4& c){
  uint32_t points = 0;
  addStyledPoint(x1,y1,c);   addStyledPoint(x2, y2,c);  addStyledPoint(x3,y3,c);
  sAddTriIndices();
  points += 3;
  numIndices.push_back(points);
  currentIndex += points;
