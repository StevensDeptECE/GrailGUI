...
void Image::addImage(float x, float y, float width, float height, float u0, float v0, float u1, float v1) {
...
void Image::combineImage(const vector<string> &images) {
  ...
  for (int i = 0; i < images.size(); i++) {
    ...
    unsigned char* img = stbi_load(file.c_str(),
    ...
    if(combinedImageData.capacity() < image_size + combinedImageData.size())
      combinedImageData.reserve(2 * (combinedImageData.capacity() + image_size));  
    combinedImageData.insert(combinedImageData.end(), img, img + image_size);
    stbi_image_free(img);
    ////combinedImageData = combinedImageData + imageData.at(i);  
  }
  stbi_write_jpg("test.jpg", w_total, h_total, STBI_rgb_alpha, combinedImageData.data(), 100);
}
