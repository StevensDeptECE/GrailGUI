...
using namespace std;
void GraphView::addVert(uint64_t v, float x, float y, uint8_t color, Point points[]) {
...
void GraphView::addEdge(uint64_t i, float x1, float y1, float x2, float y2, uint8_t initialEdgeColor) {
  mEdges.drawLine(x1, y1, x2, y2, defaultStyle);
}

void GraphView::updateVert(uint64_t v, uint8_t color) {
  // j is the position of the combined edge/verts made... should be changed later
  // e.g. addVert(), addVert()
  // ==> vert1 = pos 0
  // ==> vert2 = pos 1
  // ==> edge3 = pos 3
  mVertices.updateColors(v, colorMap[color].r, colorMap[color].g, colorMap[color].b);
//  c->init();
}

void GraphView::updateEdge(uint64_t i, uint8_t color) {
  mEdges.updateColors(i, colorMap[color].r, colorMap[color].g, colorMap[color].b);
}
...
void GraphView::addEdges(const Point vertexPos[], uint8_t initialEdgeColor) {
  ...
  for (VertexIndex_t src = 0; src < g->startIndexLen; src++) {
    ...
    for (EdgeIndex_t i = myEdgeBegin; i < myEdgeNext; i++) {
      ...
      //cout << dest << ' ';
     // if (src < 20)
      float x1 = vertexPos[src].x, y1 = vertexPos[src].y, x2 = vertexPos[dest].x, y2 = vertexPos[dest].y;
#if 0
      if (verbose) {
      ...
      //if (src < 11)
      //  printf("%d, %f, %f, %f, %f, %d\n", edgeNumber++, x1, y1, x2, y2, initialEdgeColor);
...
void circleLayout(GraphView* gv, float xc, float yc, float r,
  ...
  float vertR = pathLen/V/4.0; // leave some blank space surrounding each vertex
  double t = 0;
...
void multiCircleLayout(GraphView* gv, float xc, float yc, float rinner, float ringSpace,
  ...
  for (int i = 0; i < numRings; i++, r += ringSpace) {
    ...
    double t = 0;
    for (int j = 0; j < vertInThisRing && v < V; j++, t += vertSpacingThisRing, v++) {
      double x = xc+ r * cos(t), y = yc+r * sin(t);
...
void grid(GraphView* gv, float x0, float y0, float w, float h,
