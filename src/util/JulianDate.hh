...
public:
  bool static isLeap(uint32_t year) { //ex: 1900 is NOT a leap year (divisible by 100) 1904 is a leap year 2000 is a leap year (% 400)
    return year % 4 == 0 && year % 100 != 0 || year % 400 == 0;
  }
  JulianDate(int32_t year, uint32_t month, uint32_t day,
						 uint32_t hour = 0, uint32_t min = 0, uint32_t second = 0);
  
  JulianDate operator +(double days) const {
    return JulianDate(date + days);
  }
  ...
  double getJulDate() const {
    return date;
  }
	void extract(int32_t* year, uint32_t* mm, uint32_t* dd,
							 uint32_t* hh, uint32_t* min, uint32_t* ss) {
		double  ddate = floor(date);
		int32_t dy = ddate / 365 + ddate / 4 - ddate / 100 + ddate / 400;
		*year = epoch + dy;
