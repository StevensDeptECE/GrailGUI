...
class Buffer {
  ...
  */
  ...
  void writeStudent(Student v) {
    checkSpace(v.serializeSize());
    v.write(p);
    p += v.serializeSize();
    availSize -= v.serializeSize();
  }

  // template<>
  void writeList(List1<Student>& list) {
    write(list.getUsed());
    for (uint32_t i = 0; i < list.getUsed(); i++) {
      writeStudent(list.getData(i));
    }
  }
