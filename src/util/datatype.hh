...
enum class DataType {
  ...
  REGEX,
	...
	STRUCT8, // define new integer record identifier (number).  The number of records is 0..255
	STRUCT16, // same as record1, but can handle up to 0..65535 fields
	STRUCT32,  // 4 byte number of components, HUGE and probably useless, but here just in case
...
public:
	...
	friend ostream& operator <<(ostream& s, const Record& r) {
		DataType t = r.fieldTypes.size() < 256 ? DataType::STRUCT8 : (r.fieldTypes.size() < 65536 ? DataType::STRUCT16 : DataType::STRUCT32);
