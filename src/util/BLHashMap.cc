...
uint32_t BLHashMapBase::bytewisehash(const char s[]) const {
  uint32_t i;
...
uint32_t BLHashMapBase::bytewisehash(const char s[], uint32_t len) const {
  uint32_t i;
  uint32_t sum = s[0] ^ 0x56F392AC;
  for (i = 1; i < len; i++) {
    sum = (((sum << r1) | (sum >> (32 - r1))) ^
...
uint32_t BLHashMapBase::fasthash1(const char s[]) const {
