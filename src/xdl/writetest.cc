...
int main() {
  ...
  */
  ...
  buf3.write( uint8_t(DataType::STRUCT8) ); // write the metadata for struct8 as one byte
  string structname = "info";
  buf3.write( structname); //struct have a name
  buf3.write( uint8_t(2) ); // write the number of fields
  buf3.write( uint8_t(DataType::STRING8) );
  string fieldname = "name";
  buf3.write(fieldname); // write first the one byte length and then the string (4name)
  
  buf3.write( uint8_t(DataType::U32) );
  fieldname = "age";
  buf3.write(fieldname); // write first the one byte length and then the string  (3age)
  Buffer buf3b("test3.bin", 32768); // open data for the above metadata
  //uint8_t repeat = 2;
  string name = "Dov";
  uint32_t age = 52;
  //string name1 = "mao";
