...
CSPClient::CSPClient(char* addr, int port) : s(addr, port), pages(vector<Page>()), in(s.getIn()), out(s.getOut()) {}
...
void CSPClient::addDynamic(const DataType& type, char*& arrayp){
    switch(type) {
        ...
        case UINT4:
            ...
            break;
        case UINT8 : arrayp += sprintf(arrayp, "%lu", in.read<uint64_t>());
            break;
        case INT1:
        ...
        case INT8:
            ...
            break;
        case STRING1 : in.copyData(arrayp, type); //Is this faster than using in.read<char>?
            break;
        case STRING4 : in.copyData(arrayp, type);
            break;
        case LIST1: for (int i = 0; i < 25; i++) {
                arrayp += sprintf(arrayp, "%u", in.read<uint16_t>());
            }
            break;
        case LIST2: for (int i = 0; i < 500; i++) {
                arrayp += sprintf(arrayp, "%u", in.read<uint16_t>());
            }
            break;
        case FLOAT : arrayp += sprintf(arrayp, "%f", in.read<float>());
            break;
        case DOUBLE : arrayp += sprintf(arrayp, "%f", in.read<double>());
            break;
    }
}

//Page Request if static content is already loaded into memory
pageBuffer CSPClient::pageRequest(uint32_t pageid) { //Request dynamic data from server
    if (pageid >= pages.size()){
        cerr << "Page not in cache";
    } else{
        if (pages[pageid].isStatic()) //If no dynamic content, return static content
        ...
        char* tempArray = new char[pages[pageid].getSize() + 1]; //Need size + 1 to accomodate for \0 added on from sprintf.
        ...
        for (auto i : insertions) {
            ...
            DataType type = i.d;
            memcpy(arrayp, staticContent, offset - staticCount); //Copy over static data up to offset
        ...
        memcpy(arrayp, staticContent, pages[pageid].getStaticSize() - staticCount); //Finish copying static data
        tempArray[pages[pageid].getSize()] = '\0'; //Set last value of char array to be \0 for printing - can be removed
        return {tempArray, pages[pageid].getSize() + 1}; //Return pageBuffer of result
    }
}
...
pageBuffer CSPClient::pageRequestFile(uint32_t pageid){
    if (pageid >= pages.size()){
        ...
        //Loop through insertions, copy over static data and read in dynamic data from the buffer
        for (auto i : insertions){
        ...
        fclose(inFile);
        return {tempArray, pages[pageid].getSize()}; //Create PageBuffer object with file data and size
