...
Page::Page(const string& inputFile) {
    ...
    if (!infile.eof())
        readOffsetVector(infile);
    infile.close();
}

Page::Page(const string& staticFile, const string& insertFile): staticContent(staticFile) {
    ifstream infile;
...
uint32_t Page::dynamicSize(DataType d) {
		...
		//For now just use max possible values for all data - Just use 256 bytes for a string for now
		//TODO: Figure out what to do if we exceed max value
    switch(d) {
        case UINT1: return 3;
        case UINT2: return 5;
        case UINT4: return 10;
        case UINT8: return 20;
        case INT1: return 3;
        case INT2: return 5;
        case INT4: return 10;
        case STRING1: return 255;
        case LIST2: return 1500;
        case FLOAT: return 8;
