...
void howtoservexdl() {
  ...
  SymbolTable* st = new SymbolTable(compiler);
	Struct* point = new Struct(compiler, "Point"); // why ever create a struct if you don't know its name?
	s->addMember(XDLType::f64, "x");
	s->addMember(XDLType::f64, "y");
	s->addMember(XDLType::f64, "z");

  Struct* s = new Struct(compiler, "bogus");
  s->addMember(DataType::U32, "a", ); // could work using enumerated type (look up in the static array)
