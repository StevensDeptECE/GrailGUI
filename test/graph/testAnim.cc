...
class GraphViewer : public GLWin {
 ...
 public:
  ...
  void init() {
    ...
    // csr = new CSRGraph<uint64_t, uint64_t,
    // float>("email-Eu-core.txt_beg_pos.bin", "email-Eu-core.txt_csr.bin",
    // nullptr);
    csr = new CSRGraph<uint64_t, uint64_t, float>(
        "res/toy.dat_beg_pos.bin", "res/toy.dat_csr.bin", nullptr);
    gv = new GraphView(*csr, c, style, defaultColor);
    gv->addColor(0, 0, 0);
    gv->addColor(0, 0, 1);
    gv->addColor(0, 1, 1);
    circleLayout(gv, c->getWidth() / 2, c->getHeight() / 2, 900.0, csr->getV(),
                 0, 0);
    c->addLayer(gv);
  }

  void run() { csr->dfs(0, callback); }
};
