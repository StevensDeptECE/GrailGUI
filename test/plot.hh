...
public:
  ...
  void tickValues(std::vector<float> &X, std::vector<float> &Y);
	...
	void drawFunc(double x_min, double x_max,
								double y_min, double y_max,
								FuncOneVar f, uint32_t pixXMin,
								uint32_t pixXMax, uint32_t pixYMin,
								uint32_t pixYMax, uint32_t numPoints);
	void drawTwoFunc(double val_min, double val_max,
							double  step,
