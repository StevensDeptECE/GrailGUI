...
int main() {
  ...
  for (int i = 0; i < 26; i++) {
    sym[0] = 'a' + i;
    if (!m.get(sym, &v)) cerr << "Error: symbol " << sym << " not found\n";
    if (v != i)
  ...
  const char* addWords[] = {
      "who",        "how",  "what", "where", "cat",  "act",  "tac",  "homonym",
      "palindrome", "fuzz", "fizz", "eat",   "tea",  "ate",  "eta",  "tae",
      "tar",        "rat",  "art",  "tare",  "rate", "tear", "tree", "tee"};
  for (int i = 0; i < sizeof(symbolsNotInMap) / sizeof(char*); i++)
