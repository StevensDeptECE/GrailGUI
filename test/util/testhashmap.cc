...
private:
	...
	public:
		...
		Val val;
		Node* next;
		Node(uint32_t offset, Val v, Node* next) : offset(offset), val(v), next(next) {}
	};
	uint32_t size;
	Node**table;
...
public:
	HashMap(uint32_t sz, uint32_t symbolSize = 1024*1024) : size(sz), table(new Node*[size]) {
		size--;
  ...
  void add(const char s[], Val v) {
	...
	void hist() {
		...
		for (uint32_t i = 0; i <= size; i++) {
			uint32_t count = 0;
			for (Node* p = table[i]; p != nullptr; p = p->next)
				count++;
			if (count >= size-1)
				h[histsize-1]++;
			else
