...
/*
  Represent the state of a chess board with the ability to make moves, save to file, communicate with network...
*/
...
public:
  ...
  bool isLegalMove(int from, int to) { return true; } //TODO: for now, everything is legal
...
public:
GrailChessBoard(ChessGame* game)
  void display() const; // draw the board on the screen
};
...
public:
TextChessBoard(ChessGame* game)
  void display() const; // draw the board in ASCII
};
...
public:
  void save(const char filename[]) const;
  void load(const char filename[]);
  bool isLegalMove(int pos) { return true; } //TODO: for now, everything is legal
  void makeMove(int from, int to);
  void move(int pos);
};
