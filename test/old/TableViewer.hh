...
void TableViewer::init() {
  ...
  for (int i = 0; i < list->size(); i++, y += dy) {
    addField((*list)[i].firstName);
    x += 100;
    addField((*list)[i].lastName);
    x += 100;
    addField((*list)[i].room);
    x += 100;
    addField((*list)[i].phone);
    x += 100;
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vert.size() * 6 * 4, &vert[0],
               GL_STATIC_DRAW);
