  ...
  void init() override {
    ...
    in.displayRawRead();
	  xdl = XDLType::read(in);// First read the metadata
		...
		float objX, objY, objW, objH; // total size of graphical object which may exceed screen size
	...
	void update() override {
		renderer.update(Renderer::Dir::DOWN); // move down and redraw new page with same viewport
...
int main(int argc, char* argv[]) {
  return GLWin::init(new TestGenericTable());
}
