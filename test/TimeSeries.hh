...
private:
  ...
  uint32_t num_ticks;
public: 
  TimeSeries(std::shared_ptr<CompiledWeb> cw, float x, float y, float w, float h);
  void addPoints(std::vector<double> &y_values, std::vector<std::string> &time_values);
  void drawAxes();
   void weekLinePlot();
  ...
  void plotPoints();
  void addCandleSticks(std::vector<std::string> Times, std::vector<double> Opens, std::vector<double> Closes);
